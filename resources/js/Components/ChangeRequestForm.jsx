...
const ChangeRequestForm = ({ project, stories = [], onClose }) => {
  ...
  const handleStorySelection = (storyId, impactType, checked) => {
    ...
    Object.entries({ ...selectedStories, [storyId]: checked ? impactType : undefined })
      .forEach(([id, type]) => {
        if (type === 'IMPACTADA') {
          impacted.push(id)
        } else if (type === 'NOVA') {
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {stories.length > 0 && (
            <div className="space-y-4">
              <Label className="text-base font-semibold">User Stories Relacionadas</Label>
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-64 overflow-y-auto border rounded-lg p-4">
                {stories.map(story => (
                  <div key={story.id} className="space-y-2 p-3 border rounded">
                    ...
                    <div className="flex gap-4">
                      <label className="flex items-center space-x-2">
                        ...
                        />
                        <span className="text-sm">Impactada</span>
                      </label>
